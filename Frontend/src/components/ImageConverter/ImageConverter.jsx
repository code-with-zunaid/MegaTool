...
function ImageConverter() {
  ...
  return (
    <div className="p-5 bg-gray-100 w-full max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-4 text-center">Image Converter ({files.length})</h1>

      {/* Drag & Drop Area */}
      <div
        className="relative bg-white p-4 rounded-lg shadow-md border-2 border-dashed text-center flex items-center gap-2"
        onDrop={handleDrop}
        onDragOver={(e) => e.preventDefault()}
        style={{ minHeight: "140px" }}
      >
      ...
      {/* Buttons & Format Selection (Responsive) */}
      <div className="flex flex-col sm:flex-row gap-3 mt-3 w-full">
        ...
        <button className="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 w-full" onClick={() => alert("Convert")} disabled={loading}>
          {loading ? "Converting..." : "Convert"}
        </button>
